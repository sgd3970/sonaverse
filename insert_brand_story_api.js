...
async function insertBrandStories() {
  ...
  console.log('브랜드 스토리 더미 데이터 추가 시작...');
  
  for (let i = 0; i < brandStories.length; i++) {
    const story = brandStories[i];
    
    try {
      console.log(`\n${i + 1}. ${story.slug} 추가 중...`);
      
      const response = await fetch(`${baseUrl}/api/brand-story`, {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        console.log(`✅ ${story.slug} 추가 성공`);
      } else {
        console.log(`❌ ${story.slug} 추가 실패: ${result.error}`);
      }
      ...
      console.log(`❌ ${story.slug} 추가 중 오류: ${error.message}`);
    }
  }
  
  console.log('\n브랜드 스토리 더미 데이터 추가 완료!');
}
