...
const AdminBlogPage: React.FC = () => {
  ...
  const fetchAllBlogPosts = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/blog?published=false&pageSize=1000', {
        credentials: 'include'
      });
      if (!res.ok) throw new Error('Failed to fetch blog posts');
      const data = await res.json();
      setAllBlogPosts(data.results || []);
    } catch (err) {
  ...
  const filteredPosts = useMemo(() => {
    ...
    return allBlogPosts.filter(post => {
      const koTitle = post.content?.ko?.title?.toLowerCase() || '';
      const enTitle = post.content?.en?.title?.toLowerCase() || '';
      const koSubtitle = post.content?.ko?.subtitle?.toLowerCase() || '';
      const enSubtitle = post.content?.en?.subtitle?.toLowerCase() || '';
      ...
      <div className="bg-gray-800 rounded-lg shadow overflow-hidden border border-gray-700">
        <table className="min-w-full divide-y divide-gray-700">
          ...
          <tbody className="bg-gray-800 divide-y divide-gray-700">
            {paginationData.results.map((post) => (
